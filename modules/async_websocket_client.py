# Modified from https://github.com/Vovaman/example_async_websocket (added support for secure websockets with LetsEncrypt root)
...
URI = namedtuple('URI', ('secure', 'host', 'port', 'path'))
...
class AsyncWebsocketClient:
    ...
    def urlparse(self, uri):
        ...
        if match:
            secure, host, port, path = match.group(1), match.group(2), match.group(3), match.group(4)
            if port is None:
                port = 80 if not secure else 443

            return URI(secure, host, int(port), path)
